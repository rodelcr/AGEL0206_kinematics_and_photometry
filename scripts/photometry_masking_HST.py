"""
Interactive HST Aperture Photometry Tool

Performs elliptical aperture photometry on HST imaging data with:
...
- Noise map support (weight or RMS maps) with error propagation
...
Usage:
  python photometry_masking_HST.py
  (Edit the 'fname' variable near the top to select the target FITS file)

HST-specific notes:
  - Uses PHOTFLAM and PHOTPLAM header keywords for AB zero point calculation
  - AB_ZP = -2.5*log10(PHOTFLAM) - 21.10 - 5*log10(PHOTPLAM) + 18.69
  - If keywords are missing, prompts to load them from the full-field FITS file
"""
...
def run_photometry_math(data, header, aperture, annulus, total_mask, error_map=None):
    ...
    try:
        ...
        print("\nWARNING: PHOTFLAM/PHOTPLAM keywords missing in current file header.")
        ...
        if ans == 'y':
            ...
            try:
                with fits.open(alt_path) as hdul_alt:
                    found = False
                    for ext_id in [1, 0]:
                        if len(hdul_alt) > ext_id and 'PHOTFLAM' in hdul_alt[ext_id].header:
                            photflam = hdul_alt[ext_id].header['PHOTFLAM']
                            photplam = hdul_alt[ext_id].header['PHOTPLAM']
                            found = True
                            break
            except Exception:
                pass
    ...
    if photflam is not None and photplam is not None:
        ...
        #https://hst-docs.stsci.edu/wfc3dhb/chapter-9-wfc3-data-analysis/9-1-photometry#:~:text=WFC3%20supports%20the%20STMAG%2C%20ABMAG,21.10%20(PHOTZPT%20header%20keyword).&text=In%20the%20ABMAG%20system%2C%20the,ABMAG%20zero%20point%20is%20%2D48.6.&text=The%20relationship%20between%20ABMAG%20and,5%20log%20(PHOTPLAM)%20+%2018.692
    

        if net_flux > 0:
            ab_mag = -2.5 * np.log10(net_flux) + ab_zp
            # This is consistent with ABmag = STmag - 5*log10(PHOTPLAM) + 18.6921
            # Where STmag = -2.5*np.log10(net_flux*PHOTFLAM) - 21.10, where net_flux is in e-/s and PHOTFLAM converts to erg/s/cm2/A
...
ans_noise = input("\nDo you have a Noise/Weight map (e.g. _wht.fits or _rms.fits)? [y/n]: ").strip().lower()
if ans_noise == 'y':
    noise_path = input("Enter path to noise map: ").strip().replace("'", "").replace('"', "")
    if os.path.exists(noise_path):
        print("Loading noise map...")
        try:
            with fits.open(noise_path) as hdul_noise:
                # Assuming noise map in Ext 0 or 1
                next_id = 0
                if len(hdul_noise) > 1 and 'SCI' not in hdul_noise[0].header:
                    next_id = 1
                raw_noise = hdul_noise[next_id].data
                
                # ALIGNMENT / CUTOUT LOGIC
                if raw_noise.shape != data.shape:
                    print("Noise map shape differs from Science cutout. Reprojecting/Cutting...")
                    if HAS_REPROJECT:
                        # Reproject raw_noise to match 'header' (Science cutout)
                ...
                if raw_noise is not None:
                    # UNIT CONVERSION
                    # Weight map = 1/Variance = 1/Sigma^2  -> Sigma = 1/sqrt(Wht)
                    if 'wht' in noise_path.lower() or 'weight' in noise_path.lower():
                        print("Detected Weight map. Converting to RMS (Sigma)...")
                        ...
                        print("Assuming RMS/Sigma map.")
                        aligned_error_map = raw_noise
                    
                    # Fix NaNs after reprojection
                    aligned_error_map = np.nan_to_num(aligned_error_map, nan=0.0)
                    print("Noise map prepared.")
        except Exception as e:
            print(f"Error processing noise map: {e}")
            aligned_error_map = None
    else:
