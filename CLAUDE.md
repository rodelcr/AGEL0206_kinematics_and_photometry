...
- Seeing FWHM = 1.27" (from KCWI `GUIDFWHM` header); IFU spaxel scale = 0.30"/spaxel; 1" ≈ 7.2764 kpc at z=0.67564
- **Cosmology: Planck 2015 (H₀=67.7, Ω_m,0=0.302), adopted 2026-06-12 to match the companion lens-modeling paper (Ferrami et al.).** Was H₀=70, Ω_m=0.3 (1″≈7.04 kpc) before. Impact is a pure distance rescale (×1.0331 at z=0.67564): R_e 14.76→15.26 kpc, **all stellar masses +0.0282 dex** (exact D_L² rescale, no Bagpipes re-fit), σ_e and all angular quantities UNCHANGED. Headline numbers below are on Planck 2015 (`scripts/paper_values.py`: KPC=7.2764, DLOGM_COSMO=+0.0282 → `results/PAPER_VALUES.json`). See DRAFTING_FACTS "Cosmology switch — impact" box.
...
Systematic-budget components on the NEW reduction (post-M10 2026-05-27; **vac/air frame term dropped 2026-06-14** — deterministic per-SPS correction, <0.5 km/s σ impact (D4), and the old ±5 was the inter-SPS spread already in the pooled stat → double-count):
...
| I-shape (**14 shapes** × N=250, M11 + arc-free) | **2.29** | peak-to-peak/2 of the 14-shape spread (266.79–271.37). = the 10 maps of `run_isource_shape_sweep.py` (266.83–271.37, was ±2.27) **+ 4 arc-free PSF-matched maps** (F140W/F200LP single-Sérsic & arc-filled, convolved to 1.27″ KCWI PSF; `ishape_arcfree_pooled.py`): pooled 266.79–268.78, fall WITHIN the existing spread → I-shape 2.27→2.29 (negligible; σ_e unchanged). **Caution:** convolving the RAW arc-containing HST maps instead gives a spurious +7–8 km/s = arc leakage (`ishape_psf_convolved_test.py`), NOT used |
| arc masking (F200LP-located, 3 weights × N=500, **M11**) | **6.65** | the **general masking systematic**: arc-dilution sensitivity from the mask-weight sweep, (w00−w100)/2 = (269.69−256.39)/2 (rigorous re-derivation 2026-05-27 on NEW cube + M10 masks). **Subsumes the 4-approach arc-mask-definition spread (±4.58, larger-of-two — no double-count).** Bigger than the ±3.8 carried value because the cleaned NEW cube + Balmer-unmasked fit is more sensitive to arc dilution (more signal pixels → more leverage) |
...
| **TOTAL (symmetric)** | **11.77** | quadrature sum (R_e-source ±5.13). Computed by `scripts/paper_values.py` |
| **TOTAL (asymmetric)** | **−11.98 / +11.58** | preserves stat-side skew |
...
- Effective radius (F140W + F200LP **best-mask** CoG mean): **Re = 2.097" = 15.26 kpc** (Planck 2015; was 14.76 kpc at H₀=70) — **NEW headline (2026-06-11 "best mask throughout")**, the self-consistent best-mask (single-Sérsic + color/morph gate + WCS reg) curve-of-growth. **photutils-validated to ±0.002″** (`scripts/validate_Re_photutils.py`: our azimuthal-fill CoG reproduced by photutils `RadialProfile`; a naive masked aperture-sum biases +0.25–0.41″ high). Method systematic ±0.100″ (raw CoG 2.097 / sky-sub 1.922 / Sérsic r_eff 1.897; `scripts/Re_bestmask_reconciliation.py`, `results/Re_cog_reconciliation_bestmask.npz`). The HST 2-R_e companion mask is empty (companions only in deep JWST), so it doesn't enter R_e — global color/morph mask alone gives 2.097″ (fixed point). **Supersedes 2.305″ (expert mask)** and the older 2.61″ IFU-only value. `scripts/re_mask_sensitivity.py`.
- log(M★/M☉) = **11.5 ± 0.1 (stat) ± 0.2 (sys)** (registry 2dp 11.46) (Planck 2015; aperture-corrected total, **matched 2 R_e** at R_e=2.097″, 10% floor) — total-light stellar mass. **2026-06-16 M⋆ AUDIT (supersedes 11.50):** two coupled fixes — (1) **validated-fit aperture correction** (`scripts/aperture_correction_validated.py`): the auto `fit_sersic` was biased (r_eff 2.2–2.7″, ellip→0) and over-corrected the beyond-aperture light ~0.15 mag/band; the model shape is now fixed to the photutils-validated per-band Sérsic fit (amplitude+sky to data); (2) **quiescence-constrained SED prior** (`scripts/mstar_headline_quiescent.py`): the 4-band SED is age–dust–M/L degenerate (ΔlnZ=−0.18, `scripts/sed_quiescence_check.py`) and the flat-age prior gave a spurious SFR~57; the KCWI absorption-line spectrum (no emission) forces the passive solution. sys = masking 0.086 ⊕ apcorr-model 0.118 ⊕ SFH-prior 0.080. Passive SED: age 3.8 Gyr, SFR 8, sSFR −10.5, Z 1.25 Z☉, A_V 0.21. **Matched 2 R_e elliptical aperture** (same physical region all 4 bands), single-component Sérsic deflector (**multi-start elliptical fit, b/a≈0.80–0.86, recovered 2026-06-11 — the fitter had railed to circular; M★ unchanged**), color/morph-gated WCS-registered arc mask + **companion masking**, corrected to total by adding the Sérsic model's beyond-aperture light (GAMA/Taylor+2011 `fluxscale`). **5 estimators reported** (empirical→model, validated photom + quiescent prior, at 2 R_e, Planck 2015): raw **11.28** (empirical lower bound) · raw+apcorr **11.34** · filled **11.42** · **total 11.46 (headline)** · Sérsic-total **11.49** — now monotonic (the old auto pipeline had total 11.50 *above* Sérsic-total 11.43, the inversion that flagged the over-correction). **Sérsic-only systematic budget = ±0.19 dex** (mask-dominated ±0.125; `scripts/sersic_total_systematic.py`). **Low-M★ tail = age–dust–M/L outshining degeneracy** (all 5 estimators incl. raw; NOT the Sérsic default; `scripts/validate_sersic_fitter_synthetic.py` validates the fitter vs astropy Sersic2D). **Sérsic params per band: `scripts/sersic_parameter_table.py` (b/a 0.80–0.86, PA 4–11°, n 1.2–1.6).** **5% floor sensitivity** (`results/aperture_floor5_check.npz`): total 5%=11.52 vs 10%=11.50, Δ=+0.02 — 10% kept. **True flux errors 0.03–1.6%** (10% is a systematic floor, not the measurement error). Method refs: Taylor+2011, Sonnenfeld+2013, Graham&Driver2005. **Apcorr chain validated vs established codes (2026-06-11, `scripts/validate_apcorr_established.py`):** b_n approx ≤0.05% (vs exact gammaincinv), Sérsic total-flux formula ≤0.03% (vs render-to-20R_e), enclosed-light vs analytic incomplete-gamma (Graham&Driver05) Δ≤0.0007, empirical `F_raw` vs photutils `EllipticalAperture(exact)` ≤0.09%, sum(model_full) cutout-truncation ≤0.19% (≤0.002 mag) — no production bug. **Supersedes 11.36** (per-band-mismatched apertures) and 11.16+0.31 (2-comp over-masked). Emitted by `scripts/paper_values.py` → `results/PAPER_VALUES.json` (single source of truth — do not hand-edit). Scripts: `aperture_matched_photometry.py`, `aperture_2re_companions.py`, `sersic_total_systematic.py`. See DRAFTING_FACTS §3.2 + `HANDOFF_photometry_aperture_overhaul_2026-06-10.md`.
